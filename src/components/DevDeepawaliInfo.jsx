...
const DevDeepawaliInfo = () => {
  ...
  const previewText = renderContent().slice(0, 400);
    ...
    >
      {sections.map((sec, idx) => (
        <div key={idx} className="mb-6">
          <h2 className="text-2xl font-bold text-yellow-800 mb-2">{sec.title}</h2>
      ...
      <div className="text-center mt-6">
        <button
          ...
          className="px-6 py-3 bg-yellow-500 hover:bg-yellow-600 text-white text-lg font-semibold rounded-full transition-all duration-300 shadow-lg hover:shadow-xl"
        >
